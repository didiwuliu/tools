...
/**
 ...
 Note that it uses reserved words for table name , index name and some column
 names to test whether backquoting names works correctly.
...
function initialize($numinserts) {
  ...
  $create = 'CREATE TABLE `table` (`column` int, logtestkey int, '.
            '`order` varchar(2048), `group` varchar(100), '.
            'PRIMARY KEY (logtestkey, `column`)) '.
  ...
  // insert 1st row
  $insert = "insert into `table`(logtestkey, `column`, `order`, `group`) ".
            "values(0, 0, '%s', '%s')";
  ...
  for ($i = 0, $size = 1; $size < $numinserts; $i++,  $size *=2) {
    $insert = "insert into `table`(logtestkey, `column`, `order`, `group`) ".
              "select %d, `column`+%d, `order`, `group` from `table`";
  ...
  $index = 'create index `index` on `table`(`group`)';
  query($index, $conn);

  $index = 'create index ncindex2 on `table`(`order`)';
  query($index, $conn);
...
function deletes($start, $end) {
  ...
  for ($i = $start; $i < $end; $i++) {
    ...
    $delete = "delete from `table` where logtestkey = %d and `column` = %d";
...
function updates($start, $end) {
  ...
  for ($i = $start; $i < $end; $i++) {
    ...
    $update = "update `table` set `order` = concat('updated ', `order`), ".
              "`group` = concat('updated ', `group`) ".
              "where logtestkey = %d and `column` = %d";
...
function alter() {
  ...
  $alter = "alter table `table` ".
           "add column newcolumn varchar(10) NOT NULL DEFAULT 'newval'";
...
function osc($flags = 0) {
  ...
  $alter = 'CREATE TABLE `table` ('.
          ...
          ' `order` varchar(2048) DEFAULT NULL,'.
          ' `group` varchar(100) DEFAULT NULL,'.
          ' `newcolumn` varchar(10) NOT NULL DEFAULT "newval", '.
          '  PRIMARY KEY (`logtestkey`,`column`),'.
          '  KEY `index` (`group`),'.
          '  KEY `ncindex2` (`order`(767))'.
          ') ENGINE=InnoDB DEFAULT CHARSET=latin1';

  $osc = new OnlineSchemaChange($sock, $user, $pwd, $db, '`table`',
                                $alter,
                                null,
                                $flags,
                                50000);
...
function verify() {
  ...
  $query1 = "select min(`column`) as mintestkey, max(`column`) as maxtestkey ".
            "from `table`";
  ...
  $query2 = "select min(`column`) as mintestkey, max(`column`) as maxtestkey ".
            "from `table` where `order` like 'updated%' ";
  ...
  // verify total count
  $query3 = "select count(*) as total from `table`";
  ...
  $query4 = "select count(*) as total from `table` ".
            "where `order` not like 'updated%%' ".
            "and `column` >= %d and `column` < %d ";
