...
function osc($flags = 0) {
  ...
  $alter = 'CREATE TABLE TestOSC (testkey int, logtestkey int, '.
            ...
            'primary key (testkey, logtestkey, newcolumn) ) '.
            'ENGINE=InnoDB DEFAULT CHARSET=latin1';
...
function verify() {
  ...
  global $db;
  $query6 = "select * from information_schema.statistics ".
            "where table_name = 'TestOSC' and TABLE_SCHEMA = '%s' ".
            "and INDEX_NAME = 'PRIMARY' order by SEQ_IN_INDEX";
  $query6 = sprintf($query6, $db);
  $result6 = query($query6, $conn);
  $row6 =  mysql_fetch_assoc($result6);
  invariant($row6['COLUMN_NAME'] === 'testkey', 'Assertion failed: $row6[\'COLUMN_NAME\'] === \'testkey\'');
  $row6 = mysql_fetch_assoc($result6);
  invariant($row6['COLUMN_NAME'] === 'logtestkey', 'Assertion failed: $row6[\'COLUMN_NAME\'] === \'logtestkey\'');
  $row6 = mysql_fetch_assoc($result6);
  invariant($row6['COLUMN_NAME'] === 'newcolumn', 'Assertion failed: $row6[\'COLUMN_NAME\'] === \'newcolumn\'');
