...
/**
 Error tests Online Schema Change (OSC) by spawning 4 child processes for doing
 inserts, deletes, updates and OSC concurrently. Similar to stressTestOSC.php
 except that it leverages OSC_FLAGS_ERRORTEST, which causes random exceptions in
 OSC just before executing a SQL statement. We run OSC multiple times (about 20)
 and if all of them fail due to random exceptions, we do one final run without
 OSC_FLAGS_ERRORTEST. (We want at least one run to succeed so that we can
 do same verification steps as in stressTestOSC.php.
...
function inserts($start, $end) {
  ...
  for ($i = $start; $i < $end; $i++) {
    ...
    $insert = "insert into TestOSC(logtestkey, testkey, comment, minicomment) ".
              "values(%d, %d, '%s', '%s')";
    $insert = sprintf($insert, getlogbase2($i), $i, 'comment for'.$i,
                      'mini '.$i);
...
function deletes($start, $end) {
  ...
  for ($i = $start; $i < $end; $i++) {
    ...
    $delete = sprintf($delete, getlogbase2($i), $i);
...
function updates($start, $end) {
  ...
  for ($i = $start; $i < $end; $i++) {
    ...
    $update = "update TestOSC set comment = concat('updated ', comment), ".
              ...
              "where logtestkey = %d and testkey = %d";
    $update = sprintf($update, getlogbase2($i), $i);
...
function osc($flags = 0) {
  ...
  $alter = 'CREATE TABLE TestOSC (testkey int, logtestkey int, '.
            ...
            'index(minicomment), index(comment), '.
            'PRIMARY KEY (logtestkey, testkey)) '.
            'ENGINE=InnoDB DEFAULT CHARSET=latin1';
  global $user, $pwd, $db, $sock;
  ...
  try {
    ...
    // osc->execute raises exception only if cleanup fails. In that
    // case calls OSC again with FORCE_CLEANUP.
    if (!empty($osc)) {
      $osc->forceCleanup();
    } else {
      echo "Caught constructor exception: ". $e->getMessage(). "\n";
    }
  }
  return false;
}
...
function verify() {
  ...
  $query1 = "select min(testkey) as mintestkey, max(testkey) as maxtestkey ".
            "from TestOSC";
  $result1 = query($query1, $conn);
  $row1 =  mysql_fetch_assoc($result1);
  invariant($row1['mintestkey'] == $end_del, 'Assertion failed: $row1[\'mintestkey\'] == $end_del');
  invariant($row1['maxtestkey'] == $end_ins - 1, 'Assertion failed: $row1[\'maxtestkey\'] == $end_ins - 1');


  // verify that update has NOT modified anything < start_upd or >= end_upd
  $query2 = "select min(testkey) as mintestkey, max(testkey) as maxtestkey ".
            "from TestOSC where comment like 'updated%' ";
  $result2 = query($query2, $conn);
  $row2 =  mysql_fetch_assoc($result2);
  invariant($row2['mintestkey'] == $start_upd, 'Assertion failed: $row2[\'mintestkey\'] == $start_upd');
  invariant($row2['maxtestkey'] == $end_upd - 1, 'Assertion failed: $row2[\'maxtestkey\'] == $end_upd - 1');
  ...
  invariant($row3['total'] == $init_inserts + $inserts - $deletes, 'Assertion failed: $row3[\'total\'] == $init_inserts + $inserts - $deletes');
...
function main() {
  ...
  if ($pid4 == -1) {
    ...
    $success = false;
    for ($i=0; ($i < 30) && !$success; $i++) {
      $status = osc(OSC_FLAGS_ERRORTEST|OSC_FLAGS_DROPTABLE);
      echo "OSC[$i] returned ".($status ? 'SUCCESS' : 'FAILURE')."\n";
      $success = $success || $status;
    }

    if (!$success) {
      // do OSC without error raising. At least once OSC needs to
      // succeed for validation to pass.
      $status = osc(OSC_FLAGS_DROPTABLE);
      echo "OSC[$i] returned ".($status ? 'SUCCESS' : 'FAILURE')."\n";
    }
...
if ($argc < 3) {
  echo "Usage : php errorTestOSC.php dbname socket user pwd\n";
  exit(1);
}
...
$sleep_dml = 200; // sleep after how many DML.
