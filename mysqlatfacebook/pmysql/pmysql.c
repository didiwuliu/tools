...
void write_g_string(int fd, GString * data)
{
    ...
    while (written < (ssize_t) data->len) {
        ...
        if (r < 0) {
            g_critical("Couldn't write output data to a file: %s",
                       strerror(errno));
            exit(EXIT_FAILURE);
        }
        written += r;
...
void run_query(char *query, MYSQL *mysql, char *server_name, char *db_name, char *tag) {
    ...
    if (db_name) {
        if (mysql_select_db(mysql, db_name)) {
            g_warning("Could not select db %s on %s: %s",
                      db_name, server_name, mysql_error(mysql));
            return;
        }
    }
    ...
    do {
        ...
        if ((status = mysql_next_result(mysql)) > 0)
            g_critical("Could not execute statement on %s: %s",
                       server_name, mysql_error(mysql));
    } while (status == 0);
...
int main(int argc, char **argv)
{
    ...
    if (queryfile) {
        ...
        if (!g_file_get_contents(queryfile, &query, NULL, &error)) {
            g_critical("Could not read query file (%s): %s",
                       queryfile, error->message);
            exit(EXIT_FAILURE);
        }
    }
    ...
    */
    ...
    GThreadPool *tp = g_thread_pool_new(worker_thread, query,
                                        num_threads, TRUE, NULL);

    if (serversfile && strcmp(serversfile, "-")) {
        serversfd = fopen(serversfile, "r");
        if (!serversfd) {
            g_critical("Could not open servers list (%s): %s",
                       serversfile, strerror(errno));
            exit(EXIT_FAILURE);
        }
    } else {
        serversfd = stdin;
